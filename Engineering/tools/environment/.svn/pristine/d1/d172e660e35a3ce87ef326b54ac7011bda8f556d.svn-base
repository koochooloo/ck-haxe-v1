			...
			for (type in graphics.__commands.types) {
				...
				switch (type) {
					...
					case END_FILL:
						...
						if (hasFill && ( !Reflect.isFunction (context.isPointInPath) || context.isPointInPath (x, y))) {
							
							// CocoonJS does not support isPointInPath, so if we have a path, we just assume this was a hit
							// if we made it through Graphics.hitTest (which does the rect bounds check).
							data.destroy ();
						...
						if (hasStroke && ( !Reflect.isFunction ((context:Dynamic).isPointInStroke) || (context:Dynamic).isPointInStroke (x, y))) {
							
							// See comments in hasFill block above.
							data.destroy ();
							graphics.__canvas = cacheCanvas;
							graphics.__context = cacheContext;
							return true;
