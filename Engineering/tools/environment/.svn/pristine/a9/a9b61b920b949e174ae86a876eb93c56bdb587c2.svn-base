...
@:final class Graphics {
	...
	private function __hitTest (x:Float, y:Float, shapeFlag:Bool, matrix:Matrix):Bool {
		...
		// CanvasGraphics has a bug where the bounds sometimes become null between
		// now and when they are used in CanvasGraphics.playCommands (which is called 
		// by CanvasGraphics.hitTest). 
		// Force shapeFlag to be false so that we prefer only checking a rectangular 
		// region over potentially crashing or failing to detect any input.
		// See: https://github.com/openfl/openfl/issues/932
		shapeFlag = false;
