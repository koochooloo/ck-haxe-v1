...
class Loader extends DisplayObjectContainer {
	...
	private function loader_onComplete (event:Event):Void {
		...
		if (contentLoaderInfo.contentType.indexOf ("/json") > -1) {
           ...
           // TEMP FIX FOR SPECK / TOUPSTREAM for iOS targets?
           // Loader class is likely ignoring some HTTP error which results in it serving back empty data.
           // BitmapData will segfault trying to build image data out of this.
           // TODO - there might also be a more elegant place to put this handling, and/or a deeper source cause that would be a better place to patch.
            if ( loader.data == "" )
            {
                BitmapData_onError( "NO DATA");
                return;
            }
