...
class TextEngine {
	...
	private static var skipLineHeight:Null<Bool>;
	#end
	...
	public static function getFont (format:TextFormat):String {
		...
		#if (js && html5)
		if (skipLineHeight == null) {
			
			skipLineHeight = untyped __js__ ("(typeof navigator !== 'undefined' && typeof navigator['isCocoonJS'] !== 'undefined')"); 
		}
		#end
		...
		font += format.size + "px";
		#if (js && html5)
		if (skipLineHeight) // Cocoon can't handle line-height as part of shorthand font property yet.
		{
			font += " ";
		}
		else
