...
class NetStream extends EventDispatcher {
	...
	{
		return m_soundTransform;
	}
	
	public function set_soundTransform(tf:SoundTransform):SoundTransform
	{		
		m_soundTransform = tf;
		#if (js && html5)
		if (__video != null)
		{
			var newVolume:Float = 0;
			if (tf != null)
			{
				newVolume = m_soundTransform.volume;
			}
			__video.volume = newVolume;
		}
		#end
		return m_soundTransform;
	}
